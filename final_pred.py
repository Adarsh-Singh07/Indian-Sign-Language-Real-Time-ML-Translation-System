...
# ── Paths resolved relative to this file so the project runs on any machine ──
BASE_DIR  = os.path.dirname(os.path.abspath(__file__))
MODEL_PATH = os.path.join(BASE_DIR, 'cnn8grps_rad1_model.h5')
WHITE_PATH = os.path.join(BASE_DIR, 'white.jpg')

BOX_OFFSET = 29   # pixels added around detected hand bounding box
...
class Application:
    ...
    def __init__(self):
        # ── Hand detectors (two instances: one for full frame, one for crop) ──
        self.hd  = HandDetector(maxHands=1)
        self.hd2 = HandDetector(maxHands=1)

        # ── Spell-check dictionary for word suggestions ──
        ...
        # ── Load trained CNN model from project directory ──
        self.model = load_model(MODEL_PATH)

        # ── Pre-load white canvas image ONCE (reused every frame via .copy()) ──
    ...
    def video_loop(self):
        try:
            ...
            if not ok or frame is None:
                # Camera read failed — retry next tick
            ...
            # hands[0] = list of detected hands, hands[1] = annotated image
            hand_list = hands[0] if isinstance(hands, tuple) else hands
            if hand_list:
                hand = hand_list[0]
                x, y, w, h = hand['bbox']
                # Clip crop to frame boundaries to avoid negative-index crashes
                y1 = max(0, y - BOX_OFFSET)
                y2 = min(cv2image_copy.shape[0], y + h + BOX_OFFSET)
                x1 = max(0, x - BOX_OFFSET)
                x2 = min(cv2image_copy.shape[1], x + w + BOX_OFFSET)
                image = cv2image_copy[y1:y2, x1:x2]

                # Fresh copy of white canvas from pre-loaded template
                ...
                if image.size > 0:
                    ...
                    if hand_list2:
                        handmap = hand_list2[0]
                        self.pts = handmap['lmList']

                        # Centre the skeleton drawing within the 400x400 white canvas
                        ox = ((400 - w) // 2) - 15
                        oy = ((400 - h) // 2) - 15
            ...
            print("[ERROR in video_loop]\n", traceback.format_exc())
        finally:
            # Schedule the next frame — 1ms delay keeps it as fast as possible
            self.root.after(1, self.video_loop)

    # ──────────────────────────────────────────────────────────────────────
    # Helper: Euclidean distance between two 2D landmark points
    # ──────────────────────────────────────────────────────────────────────
    def distance(self, x, y):
        return math.sqrt(((x[0] - y[0]) ** 2) + ((x[1] - y[1]) ** 2))

    def _draw_skeleton(self, white, ox, oy):
        """Draw hand landmark skeleton on the white canvas.
        ox, oy = pixel offsets to centre the skeleton in the 400x400 image.
        """
        pts = self.pts
        # Finger segments
    ...
    def predict(self, test_image):
        ...
        # ── Space gesture: open palm sideways (index+pinky up, middle+ring down) ──
        # BUG FIX: was `or 'E' or 'S'` which is always True due to truthy strings.
        # Corrected to use `in [...]` membership test.
        ...
        # ── Next gesture: all fingers extended + thumb tucked ──
        if ch1 in ['E', 'Y', 'B']:
        ...
        # ── Backspace gesture: fist with thumb raised above all fingers ──
        ...
        # ── Word suggestions via pyenchant spell-checker ──
...
# ── Entry point ──
# Using __main__ guard so this file can be safely imported
# without triggering the GUI (e.g., for unit testing).
if __name__ == "__main__":
    print("Starting Indian Sign Language — Real-Time ML Translation System...")
    app = Application()
    app.root.mainloop()
